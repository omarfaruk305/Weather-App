...
class NlSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        """
        table = response.xpath('//*[@id="mw-content-text"]/div[1]/table[1]/tbody/tr')
        for city in table : 
    ...
    def population(self,response):
        ...
        population_rows =  response.xpath('//*[@id="mw-content-text"]/div[1]/table[1]').css('tbody tr ')
        for row in population_rows:
            ...
            if row.css('td a ::attr(title)').get() == 'Bevolking':
                population = (row.css('td::text').getall()[1].rstrip('\n').replace('.',''))
